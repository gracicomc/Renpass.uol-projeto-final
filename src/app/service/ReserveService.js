...
class ReserveService {
  async create(rentalId, payload) {
    ...
    const { date_start, date_end, id_car, id_user } = payload;

    const car = await FleetRepository.list({ id_car });
    ...
    const getDailyValue = fleet[0].daily_value;
  ...
  async getById(rentalId, payload) {
    const rental = await RentalRepository.getById(rentalId);
    if (!rental) throw new NotFound(`This Rental ID '${rentalId}' is not registered`);
  ...
  async deleteById(rentalId, payload) {
    const rental = await RentalRepository.getById(rentalId);
    if (!rental) throw new NotFound(`This Rental ID '${rentalId}' is not registered`);
